...
const Vertex H1(-3.0, 2.0, 2.0);
    ...
    void drawQuad(const Vertex& v1, const Vertex& v2, const Vertex& v3, const Vertex& v4, bool withTexture = false) {
    ...
    if (withTexture) {
        // Текстура накладывается один раз на всю грань
...
void drawFigure() {
    ...
    drawQuad(A, B, B1, A1, texturing);
    drawQuad(B, C, C1, B1, texturing);
    drawQuad(C, D, D1, C1, texturing);
    drawQuad(D, E, E1, D1, texturing);
    drawQuad(E, F, F1, E1, texturing);
    drawQuad(F, G, G1, F1, texturing);
    drawQuad(G, H, H1, G1, texturing);
    drawQuad(H, A, A1, H1, texturing);
    ...
    for (int i = 0; i < num_triangles; i++) {
        ...
        glTexCoord2d(0.0, angle / PI);
        glVertex3d(x, y, 0.0);
        glTexCoord2d(1.0, angle / PI);
        glVertex3d(x, y, 2.0);
        glTexCoord2d(1.0, next_angle / PI);
        glVertex3d(nx, ny, 2.0);
        glTexCoord2d(0.0, next_angle / PI);
